#!/usr/bin/env bash
# Two product lifetimes in one container:
#   phase neg -- --lora-dir points at the benign package, nothing may fire
#   phase pos -- the DEFAULT models/Lora holds the attacker package
# webui.sh (not launch.py) is the launcher on purpose: it exports SD_WEBUI_RESTART
# and re-execs, which is what makes "Apply and restart UI" a genuine self-restart
...
echo "mbe2e/v/a1111.sshs_hash_xss_staged_rce" > /evidence/logs/image.txt
printf '%s\n' "MBE2E-SENTINEL a1111.sshs_hash_xss_staged_rce" > /evidence/logs/SENTINEL
...
{ echo "phase neg: bash webui.sh $ARGS --lora-dir /app/webui/models/Lora_neg";
  echo "phase pos: bash webui.sh $ARGS   (default --lora-dir = models/Lora)"; } > /evidence/logs/command.txt
...
RC_NEG=$?
stop_product
cp /evidence/logs/product_stdout.log /evidence/logs/product_stdout_neg.log
: > /evidence/logs/product_stdout.log
# A1111 caches safetensors metadata keyed by the path RELATIVE to the lora dir
# (modules/cache.py + extensions-builtin/Lora/network.py:31-37).  The benign phase
# populated that cache; a victim who has only ever seen the attacker package would
# not have it.  Remove the derived cache so phase pos starts from a clean read.
rm -f /app/webui/cache.json

echo "=== phase pos ===" >> /evidence/logs/product_stdout.log
# shellcheck disable=SC2086
bash webui.sh $ARGS >> /evidence/logs/product_stdout.log 2>&1 &
...
echo "rc_neg=$RC_NEG rc_pos=$RC_POS rc_merge=$RC treehash=$TREEHASH pin_ok=$MBE2E_PIN_OK"
exit "$RC"
